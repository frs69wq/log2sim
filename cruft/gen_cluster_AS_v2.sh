...
##################################################################
# SEs are connected to the Backbone of AS using two separated    #
# links (up, down). The bandwidth of each one is equal to max    #
# value of file size/time to transfer.                           #
################################################################## 
...
awk -F',' '{gsub("ms","",$4); \
 if ($NF == "0") \
   {bw=$3/$4;if (bw > arr_bw[$2]){arr_bw[$2]=bw};} \
 else \
   {bw=$3/$4;if (bw > arr_bw[$1]){arr_bw[$1]=bw};}}
 END{\
   for (id in arr_bw) 
      {print id" "arr_bw[id]} \
 }'| awk '{ \
