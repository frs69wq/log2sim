...
sources_by_gate_job = reshape(download_sources_gate, timevar = "File", idvar="JobId", direction = "wide")

gate_arguments <- merge(merge(uploads_gate, start_and_compute_times), sources_by_gate_job)

uploads_merge <- raw_transfers[raw_transfers$UpDown == 1 & raw_transfers$JobType == "merge" & 
                                 raw_transfers$FileSize > 12, c(2,3,5,7)]
download_sources_merge <- head(raw_transfers[raw_transfers$UpDown == 2 & raw_transfers$JobType=="merge",c(2,3)], n=2)
...
for (version in 1:2){
  ...
  for (i in 1:nrow(gate_arguments)){
    t$addTag("process", attrs=c(host=gate_arguments[i,2], fun="Gate", start_time=gate_arguments[i,5]), close=FALSE)
  ...
  if (version == 1){
    f=paste(output_dir,"/deployment_",workflow_name,".xml", sep="")
  } else {
