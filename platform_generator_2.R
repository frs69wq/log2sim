...
get_transfers <- function(file_name){
  ...
  # QUESTION: To be expressed in "bits per seconds" or in "Kilobytes per seconds" ???
  df$Bandwidth_in_bps  <- (8*df$FileSize)/pmax(0.001,(df$Time-1))
  df$Bandwidth_in_KBps <- (df$FileSize/pmax(0.001,(df$Time-1)))/1000
...
correct_bandwidth <-function(df){
  ...
  for(i in 1:nrow(df)){
    ...
    others <- subset(df, Link == df[i,]$Link & (Start + 1) <= end & End >= begin)
    if (n_interval == 1){
      # if there is only one interval to cover, we're done
      df[i,]$concurrency <- nrow(others)
    } else {
      cur_concurrency <- 0
      if (nrow(others) > 1) {
        ...
        for(s in 1: n_interval){
          ...
          # this interval
          cur_concurrency <- cur_concurrency + 1/(nrow(subset(others, (Start +1) <= end & End >= begin)))    
          # update begin for the next round
          begin <- end
        }
        # End of formula: N/sum_{i=1}^{N}(1/C_i)
        df[i,]$concurrency <- n_interval/cur_concurrency
      }
    }
  }
  df$Corr_Bandwidth_in_bps  <-  df$Bandwidth_in_bps* df$concurrency
  df$Corr_Bandwidth_in_KBps <-  df$Bandwidth_in_KBps* df$concurrency
  df
}

get_bandwidths_by_link <- function(df){
